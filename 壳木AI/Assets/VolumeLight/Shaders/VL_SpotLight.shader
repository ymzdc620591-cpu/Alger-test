	...
	SubShader
	{
		...
		Pass
		{
			...
			real4 frag(v2f i) : SV_Target
			{
				...
				real rayStartOffset = random01(i.uv) * stepLength * _JitterVolumetric / 10;
				...
				for (real j = 0; j < _Steps - 1; j++)
				{
					...
					//if it is in light
					if (shadowMapValue > 0) {
						real kernelColor = ComputeScattering(dot(rayDirection, _LightDirection));
				...
				return real4(accumFog, accumFog, accumFog, 1) * _Intensity * _LightColor;
			}
			ENDHLSL
		}
	}
